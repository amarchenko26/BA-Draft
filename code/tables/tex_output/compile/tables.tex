...
\input{../individual_tables/price}
	...
	\item {\it Note:} The dependent variable is the log price of the listing. The omitted category is white males. The unit of observation is a listing. The sample is the sample of listings across 7 US cities. Model 1 is the baseline effect of host demographics on price. Model 2 controls for listing location to the neighborhood level. Model 3 adds listing characteristics such as property type, time on market, number of bedrooms, availability, etc. Model 4 adds host characteristics such as response and acceptance rates, measures of host effort, Superhost status, etc.  
\end{tablenotes}
\end{table}
...
\begin{table}[htbp]\centering
	...
	\begin{tabular}{l*{5}{c}}
		\hline\hline
		\input{../individual_tables/price_no_int}
		\hline\hline
		\multicolumn{5}{l}{\footnotesize Standard errors in parentheses}\\
		\multicolumn{5}{l}{\footnotesize \sym{*} \(p<0.05\), \sym{**} \(p<0.01\), \sym{***} \(p<0.001\)}\\
	\end{tabular}
...
\begin{landscape}
	\begin{table}[htbp]\centering
		...
		\begin{tabular}{l *{4}{c}}
			...
			\cmidrule(r){2-5}\\
			\input{../individual_tables/reviewer_mean_reg_no_int}	
			\hline\hline
			\multicolumn{5}{l}{\footnotesize Standard errors in parentheses}\\
			\multicolumn{5}{l}{\footnotesize \sym{*} \(p<0.05\), \sym{**} \(p<0.01\), \sym{***} \(p<0.001\)}\\
		\end{tabular}
